...
kubectl get pods --all-namespaces | grep -E "(monitoring|argocd|vault|vprodb|vpromq01)" > cluster-state-backup.txt
...
echo "✅ Vault encrypted secrets: PRESERVED (Persistent storage)"
echo "✅ Database & RabbitMQ encrypted passwords: PRESERVED in Vault"
echo "✅ All configurations will restore on startup"
echo "✅ LoadBalancer URLs remain the same"
echo ""
echo "📋 Cluster state backed up to: cluster-state-backup.txt"
